...
namespace Gefest
{
	public class frmPrintrepReferenceOnSubscriptionServiceRuk : Form
	{
		...
		public frmPrintrepReferenceOnSubscriptionServiceRuk()
		{
		...
		private void Rep(long IDPeriod, DateTime dBegin)
		{
			try
			{
				try
				{
					...
					string[] str = new string[] { "@idPeriod", "Month" };
					string[] strArrays = str;
					str = new string[] { IDPeriod.ToString(), null };
					DateTime value = this.txtdEnd.Value;
					str[1] = value.ToString("MMMM yyyy");
					string[] strArrays1 = str;
					string str1 = string.Concat(Depot.oSettings.ReportPath.Trim(), "repReferenceOnSubscriptionServiceRuk.rpt");

...
namespace Gefest
{
	public class frmFilterClaimDocs : Form
	{
		...
		private void frmFilterClaimDocs_Closing(object sender, CancelEventArgs e)
		{
			Tools.SaveWindows(this);
		}

		private void frmFilterClaimDocs_Load(object sender, EventArgs e)
		{
			Tools.LoadWindows(this);
			this.cmbStatus.AddItem("Любое состояние");
			this.cmbStatus.AddItem("Сформирована");
			this.cmbStatus.AddItem("Вручена");
		...
		{
			ResourceManager resourceManager = new ResourceManager(typeof(frmFilterClaimDocs));
			...
			base.Name = "frmFilterClaimDocs";
			this.Text = "Фильтр";
			base.Closing += new CancelEventHandler(this.frmFilterClaimDocs_Closing);
			base.Load += new EventHandler(this.frmFilterClaimDocs_Load);

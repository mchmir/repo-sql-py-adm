...
namespace Gefest
{
	public class frmGetPlombFile : Form
	{
		...
		private void cmdApply_Click(object sender, EventArgs e)
		{
			try
			{
				...
				UslugiVDGOs uslugiVDGO = new UslugiVDGOs();
				uslugiVDGO.Load();
							...
							if (contract[0] != null)
							{
								...
								if (item.SubItems[11].Text != null)
								{
									...
									if (gmeter != null)
									{
										...
										{
											gmeter.PlombNumber1 = item.SubItems[7].Text;
											str = "пломба 1 установлена";
										}
										if (item.SubItems[8].Text != "")
										{
											gmeter.PlombNumber2 = item.SubItems[8].Text;
											str = (str.Length <= 0 ? "пломба 2 установлена" : "пломбы 1, 2 установлены");
										}
										...
										gmeter.IDTypePlombWork = 1;
										if (!item.Checked)
										{
											gmeter.PlombMemo = "опломб без начисл";
										}
										else
										{
										...
										if (gmeter.Save() == 0)
										{
											if (item.Checked)
											{
												Document document = new Document()
												{
													...
													oTypeDocument = Depot.oTypeDocuments.item((long)24)
												};
												if (!((item.SubItems[7].Text == "") | (item.SubItems[8].Text == "")))
												{
													document.DocumentAmount = Convert.ToDouble(uslugiVDGO.item((long)18).Value);
												}
												else
												{
													document.DocumentAmount = Convert.ToDouble(uslugiVDGO.item((long)19).Value);
												}
												document.DocumentDate = Convert.ToDateTime(item.SubItems[5].Text);
												document.Note = "Опломбировка ПУ";
												if (document.Save() == 0)
												{
													...
													if (pD.Save() == 0)
													{
														pD = document.oPDs.Add();
														pD.oTypePD = Depot.oTypePDs.item((long)35);
														pD.oDocument = document;
														if (!((item.SubItems[7].Text == "") | (item.SubItems[8].Text == "")))
														{
															pD.Value = Convert.ToString(uslugiVDGO.item((long)18).get_ID());
														}
														else
														{
															pD.Value = Convert.ToString(uslugiVDGO.item((long)19).get_ID());
														}
														if (pD.Save() == 0)
														{
															Balance currentPeriod = item1.CurrentBalance(accounting.item((long)6));
															if (currentPeriod == null)
															{
																currentPeriod = item1.oBalances.Add();
																currentPeriod.oAccounting = accounting.item((long)6);
															...
															if (!((item.SubItems[7].Text == "") | (item.SubItems[8].Text == "")))
															{
																...
																amountBalance.AmountBalance = amountBalance.AmountBalance - Convert.ToDouble(uslugiVDGO.item((long)18).Value);
															}
															else
															{
																Balance balance = currentPeriod;
																balance.AmountBalance = balance.AmountBalance - Convert.ToDouble(uslugiVDGO.item((long)19).Value);
															}
															if (currentPeriod.Save() == 0)
															{
																Operation operation = new Operation()
																{
																	DateOperation = Convert.ToDateTime(item.SubItems[5].Text)
																};
																if (!((item.SubItems[7].Text == "") | (item.SubItems[8].Text == "")))
																{
																	Operation amountOperation = operation;
																	amountOperation.AmountOperation = amountOperation.AmountOperation - Convert.ToDouble(uslugiVDGO.item((long)18).Value);
																}
																else
																{
																	Operation amountOperation1 = operation;
																	amountOperation1.AmountOperation = amountOperation1.AmountOperation - Convert.ToDouble(uslugiVDGO.item((long)19).Value);
																}
																operation.oBalance = currentPeriod;
																operation.oDocument = document;
		...
		private void cmdOpen_Click(object sender, EventArgs e)
		{
			...
			{
				Filter = "Реестр опломбировки (*.xlsx)|*опломбировки*.xlsx"
			};
			if (openFileDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
			{
				...
				this.Text = string.Concat("Прием реестра опломбировки ПУ: ", this.filepath);
				base.Invalidate();
		...
		private void frmGetPlombFile_Closing(object sender, CancelEventArgs e)
		{
			Tools.SaveWindows(this);
		}

		private void frmGetPlombFile_Load(object sender, EventArgs e)
		{
			try
			{
		...
		private void InitializeComponent()
		{
			...
			this.label5.Text = "- на ПУ есть пломба;";
			...
			base.Name = "frmGetPlombFile";
			base.StartPosition = FormStartPosition.CenterScreen;
			this.Text = "Прием реестра опломбировки ПУ";
			base.Closing += new CancelEventHandler(this.frmGetPlombFile_Closing);
			base.Load += new EventHandler(this.frmGetPlombFile_Load);
								...
								if (gmeter != null)
								{
									...
									try
									{
										if (enumerator.MoveNext())
										{
											...
											if (activeGMeter.PlombNumber1.Length > 0 && activeGMeter.PlombNumber1.Length > 0 & item.SubItems[7].Text.Length > 0)
											{

...
MyFolders=Array("Мои рисунки","My Scans","Downloads","Загрузки","Моя музыка")
Public CompUsers
Public CompUsers2
Public CompUsers3
	...
	if len(Name)<0 Then
	    ...
	    for each objFolder in objFSO.GetFolder(Src).Subfolders
	    	...
	    	if Need = 1 then
	        	...
	        	LogStream.WriteLine "--Папка " & objFolder.Name & " не подлежит копированию"
	    	end if
...
'	        	LogStream.WriteLine "--Папка " & objFolder.Name & " скопирована."
'	        end if
'	        if not objFolder.Name="Загрузки" then
'	        	CopyDir Src+"\"+objFolder.Name ,objFolder.Name, Dst+"\"+objFolder.Name
'	        	LogStream.WriteLine "--Папка " & objFolder.Name & " скопирована."
'	        end if
	    next

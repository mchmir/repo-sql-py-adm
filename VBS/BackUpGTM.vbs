...
Set objScriptExec = WshShell.Exec("%comspec% /c ping.exe -n 2 GTM-2")
	...
	if InStr(strPingResults, "ttl=") Then
		...
		LogStream.WriteLine "!!!Не пингуется GTM-2. Не могу копировать."
	end if
''--
Set objScriptExec = WshShell.Exec("%comspec% /c ping.exe -n 2 buh-main")
	...
	if InStr(strPingResults, "ttl=") Then
		'копируем папки
		''
		strWorkFolder="\\Buh-main\D$\1Cv8\Базы\"
		''профит
		...
		LogStream.WriteLine "!!!Не пингуется Buh-main. Не могу копировать."
	end if
''--
Set objScriptExec = WshShell.Exec("%comspec% /c ping.exe -n 2 buh-11")
	...
	if InStr(strPingResults, "ttl=") Then
		...
		LogStream.WriteLine "!!!Не пингуется Buh-11. Не могу копировать."
	end if

...
begin
  ...
  begin
    ...
    BACKUP DATABASE [Gefest] TO  DISK = @path WITH NOFORMAT, INIT,  NAME = N'Gefest backup', SKIP, NOREWIND, NOUNLOAD,  STATS = 10
      ...
      values ('ActionForPeriod','Сделали BackUp',GetDate())
    ...
    --определим дату начала нового периода
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Вернулись из CloseNowPeriod',GetDate())
    ...
    group by d.iddocument, d.idcontract , d.documentamount, d.documentdate, d.IdTypeDocument, d.IdBatch,
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Подготовили документы для перепроведения',GetDate())
    --
    ...
    inner join #tmpDoc td on td.iddocument = d.iddocument
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Перенесли Документы в новый период',GetDate())
    ...
    where  dbo.DateOnly (BatchDate)>=@FirstDate
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Перенесли Пачки в новый период',GetDate())
    ...
    deallocate Loc
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Перепровели документы',GetDate())
  ...
  --Exec(@Q)
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Начали работать с Балансами',GetDate())
    ...
    begin
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod', 'Удаляем ненужные дубли в Балансах ' + cast(@i as varchar(3)), GetDate())
    ...
    drop Table #TempBB
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod', 'Удаляем ненужные дубли в Балансах - Выполнено #' + cast(@i as varchar(3)),GetDate())
    ...
    insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
    values ('ActionForPeriod','Исправление привязки операций в Балансах - Выполнено',GetDate())

    -------------------------------------------------------------------------------
      --логируем
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Пересчет балансов пред.периода - начали',GetDate())
    ...
    --declare @IdPeriod int
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Пересчет балансов пред.периода - Закончили',GetDate())

      --логируем
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Пересчет балансов нового периода - начали',GetDate())
    ...
    exec dbo.spRecalcBalancesRealOnePeriod @IdPeriod
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Пересчет балансов нового периода - Закончили',GetDate())

        --логируем
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Закончили пересчет балансов.',GetDate())

  ----------------------------------------------------------------
  --------------------------------07012021--------------------------------
  /*--логируем
  insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
  values ('ActionForPeriod','Перепроводим баланс по документам с измененным периодом',GetDate())
  declare @IdPeriodNow int
  set @IdPeriodNow = dbo.fGetNowPeriod()
  declare @IdPeriodPre int
  set @IdPeriodPre = dbo.fGetPredPeriod()

       DECLARE cursC CURSOR LOCAL FAST_FORWARD FOR

               SELECT  DISTINCT d.IDContract FROM Document d INNER JOIN Operation o ON d.IDDocument = o.IDDocument WHERE d.IDPeriod =  dbo.fGetNowPeriod()
            ...
            OPEN cursC
                ...
                WHILE @@FETCH_STATUS = 0 BEGIN
                  -------------------------------------------------------------------------------------
                    ...
                    EXEC spRecalcBalances @idContract,  @IdPeriodPre
                    EXEC spRecalcBalancesRealOnePeriodByContract  @idContract,  @IdPeriodPre
                    EXEC spRecalcBalances @idContract,  @IdPeriodNow
                    EXEC spRecalcBalancesRealOnePeriodByContract  @idContract,  @IdPeriodNow
  ...
  ----------------------------------------------------------------*/
    ...
    insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
    values ('ActionForPeriod','Наполнение PD - начали',GetDate())
    ...
    and pd.value=2 and idtypepd=22)dd where vv is null
      ...
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod','Наполнение PD - закончили', GetDate())

      --логируем
      insert into dbo.ClosePeriodLog (SPName,StepName,DateExec)
      values ('ActionForPeriod', 'Период закрыт', GetDate())

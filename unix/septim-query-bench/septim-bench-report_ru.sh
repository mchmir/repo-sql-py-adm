...
# =====================[ ЛЕГЕНДА К ОТЧЁТУ ]=====================
# ВРЕМЯ И СТРОКИ
#  cold_ms    — "холодный" прогон: время EXPLAIN (ANALYZE) первого запуска
#               в текущей сессии после DISCARD PLANS/SEQUENCES. мс.
#  hot_ms     — "горячий" прогон: время второго запуска того же запроса.
#               Обычно ниже/равно cold_ms за счёт прогретых кэшей.
#  rows_plan  — фактическое число строк на корневом узле плана
#               (.[0].Plan."Actual Rows" из EXPLAIN JSON) для hot-прогона.
#  rows_real  — COUNT(*) того же запроса. Должно совпадать с rows_plan,
#               если нет внешних LIMIT/фильтров.
#
# BUFFERS (суммы по всем узлам плана hot-прогона; единица — блок 8 КБ)
#  shared_hit     — чтения из буферного кэша PostgreSQL (без физдиска).
#  shared_read    — физические чтения с диска (чем меньше, тем лучше).
#  temp_read      — чтения из временных файлов (спиллы сортов/хэшей).
#  temp_written   — записи во временные файлы (признак недостаточного work_mem).
#  Преобразование в МБ/ГБ: 1 блок = 8 КБ,  MB = blocks * 8 / 1024,  GB = MB / 1024.
#
# СРАВНЕНИЕ ДАННЫХ (между двумя снапшотами, здесь: wdn ↔ wds)
#  wdn_rows / wds_rows — размер множеств строк (по числу уникальных хэшей).
#  intersection        — |WDN ∩ WDS|, сколько строк совпало у обеих версий.
#  wdn-wds             — |WDN \ WDS|, строки, которые есть только в WDN.
#  wds-wdn             — |WDS \ WDN|, строки, которые есть только в WDS.
#  jaccard             — коэффициент Жаккара: |A ∩ B| / |A ∪ B|
#                        = intersection / (wdn_rows + wds_rows - intersection).
#                        Диапазон [0..1]. 1.0 = наборы полностью идентичны;
#                        ближе к 0 — сильные различия.
#
# ПРАКТИЧЕСКАЯ ИНТЕРПРЕТАЦИЯ
#  • Если hot_ms у новой версии >> чем у старой, и при этом shared_* в разы выше,
#    причина деградации почти наверняка в большем объёме обрабатываемых страниц
#    (шире JOIN/агрегаты/меньше селективности).
#  • temp_written/temp_read > 0 — были спиллы во временные файлы;
#    попробуйте увеличить work_mem для сессии/запроса либо сделать план менее "широким".
#  • jaccard=1.000000 и wdn-wds=0, wds-wdn=0 — данные формально эквивалентны.
#    Если jaccard < 1, смотрите *_minus_*_sample.csv — там примеры расхождений.
# ===============================================================
set -euo pipefail
...
# --- Достаём ЧИСТЫЙ JSON-массив из "табличного" EXPLAIN вывода psql ---
clean_explain_json() {  # $1 = путь к *_explain_*.json
  awk '
    BEGIN{start=0; depth=0}
    {
      line=$0
      sub(/[[:space:]]+\+$/,"", line)                    # убираем хвост " +"
      if (!start) {                                       # ждём первую строку с '['
        if (line ~ /^\[/) { start=1 }
        else { next }
      }
      if (line ~ /^Time: / || line ~ /^Timing is /) next  # выбрасываем таймерные строки
      print line
      # баланс скобок, чтобы остановиться ровно на закрывающей "]" верхнего уровня
...
# --- Считаем метрики из EXPLAIN JSON (stdin = чистый JSON) ---
explain_stats() {
  jq -r '
    def buf:
      (.[0].Plan
...
# --- Надёжно достаём COUNT(*) (первая строка, содержащая только число) ---
count_value() { awk '/^[[:space:]]*[0-9]+[[:space:]]*$/{print $1; exit}' "$1"; }

# --- Хэши строк из CSV-снапшотов ---
...
# === Сравнение данных: только wdn ↔ wds ===
...
echo
echo "=== HOT BUFFERS (сумма по плану) ==="
...
echo "Примеры расхождений: wdn_minus_wds_sample.csv, wds_minus_wdn_sample.csv (до 50 строк каждый)."

# --- JSON-отчёт (надёжная сборка) ---
...
echo "Готово: report_summary.json, *.set, *_sample.csv в $DIR"

# === Так как папка в процессе изменения, то сначала надо сделать снимок, затем заархивировать весь каталог /home/mikhailchm/septim-query-bench/report ===
LABEL="${1:-report}"
...
# 1) Создать стабильный снимок в временном каталоге (обрабатывает изменения в реальном времени)
SNAPROOT="$(mktemp -d)"
...
if command -v rsync >/dev/null 2>&1; then
  # rsync сохраняет perms/times/hardlinks и может выполнять --delete для обеспечения согласованности
...
# 2) Скомпилируйте снимок (предпочтительно tar; в качестве запасного варианта cpio+gzip)
if command -v tar >/dev/null 2>&1; then
  # записывать архив рядом с OUTDIR (не внутри SRC), с относительными путями

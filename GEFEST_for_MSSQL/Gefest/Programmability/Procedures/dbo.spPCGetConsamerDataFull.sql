...
CREATE PROCEDURE [dbo].[spPCGetConsamerDataFull] (@Account varchar(20),@year int,@Month int)
AS
BEGIN
	SET NOCOUNT ON;
declare @IdGRU int
...
update #tmpChetIzvehe
set DateGetIndication=isnull((select top 1 convert(datetime,qq.datedisplay,20) from  indication qq  with (nolock) where qq.display=c.GetIndication and c.idgmeter=qq.idgmeter order by datedisplay desc),GetDate())
from #tmpChetIzvehe c

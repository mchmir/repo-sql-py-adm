...
CREATE procedure [dbo].[repCountNoticeGM] (@IdGRU int, @IdPeriod int) as 
---------------*************Счет-извещение**********------------------
--declare @IdGRU int
...
if @IdPeriod=122
begin
exec dbo.repCountNoticeGMNewTariff @IdGRU, @IdPeriod
end
else
begin
...
------------------------------------changed 29012019-------------------------------------------
CASE WHEN np.isJuridical = 0 THEN 
np.NSurname+' '+left(ltrim(isnull(np.NName,'')),1)+'. ' +left(ltrim(isnull(np.NPatronic,'')),1)+'.'
ELSE np.Surname+' '+left(ltrim(isnull(np.Name,'')),1)+'. ' +left(ltrim(isnull(np.Patronic,'')),1)+'.' end  FIO,
-------------------------------------------------------------------------------    
...
inner join person np with (nolock)  on np.idperson=c.Idperson 
 ------------------ Только юридические лица----- changed 29012019-------------------------------------------------------------------------------
--AND (np.isJuridical = 1 OR (np.isJuridical = 0 AND c.IDContract IN (885122,885123,885124,885125,885126,885127,885128,885129))) -- 0, только физические лица
--AND c.Account IN (3151062)
 ------------------------------------------------------------------------------------------------------------------------------
...
----------------------------------------------------
   ,np.Surname, np.Name, np.Patronic, np.isJuridical
----------------------------------------------------
...
left join (select idcontract 
            ...
            ) c on c.idcontract=t.idcontract

...
and  d.IdPeriod=@IdPeriod and (d.idtypedocument=1  or d.idtypedocument=3)
  -----*******************************************************************************************-------------------------------------
INNER JOIN Person p WITH (NOLOCK) ON p.IDPerson = c.IDPerson AND p.isJuridical = 0 AND c.IDContract NOT IN (885122,885123,885124,885125,885126,885127,885128,885129) -- только физические лица
...
inner join (select idcontract from gobject where idstatusgobject=2 --and idcontract=@IDContract
  ) obj on obj.idcontract=#tmpChetIzvehePU.idcontract
...
--from accounting ac
---------------------------------------------------------
select ac.idaccounting, ac.nname,с.idcontract,
isnull( dbo.fGetLastBalance(@IdPeriodPred, с.idcontract, ac.idaccounting),0) as BalanceBegin,
convert(float, 0.00) as SummOplat,
convert(float, 0.00) as SummaNach,
isnull( dbo.fGetLastBalance(@IdPeriod, с.idcontract, ac.idaccounting),0) as BalanceEnd
into #tmpAcc
FROM Contract с, accounting ac
WHERE с.IDContract IN (SELECT c.IDContract  
                        FROM GRu g  
                        left JOIN GObject gt   ON gt.IDGru = g.IDGru AND g.IDGru = @IdGRU
...
from #tmpAcc c
inner join (select sum(ISNULL(o.amountoperation,0)) AO, b.idaccounting, b.IDContract
from Document f  with (nolock) 
...
group by b.idaccounting, b.IDContract) qq on qq.idaccounting=c.idaccounting AND  qq.IdContract=c.IdContract

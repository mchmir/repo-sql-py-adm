...
CREATE PROCEDURE [dbo].[repAccountUsl] (@IdContract int, @dBegin datetime,  @dEnd datetime) AS
------------*****************Отчёт карточка абонента***************--------------------------------

--declare @idcontract int
--set @IdContract=(select idcontract from contract where account='3282174')
--declare @dBegin datetime
--declare @dEnd datetime
--set @dBegin='2005-05-01'
--set @dEnd='2009-02-02'
declare @DTPeriod as int
set @DTPeriod=122
...
Imya nvarchar(100)collate Cyrillic_General_CI_AS, Account varchar(20), NameDoc varchar(200)collate Cyrillic_General_CI_AS,
...
typeindication varchar(100)collate Cyrillic_General_CI_AS,typedocument int, AmountNach float, AmountPay float,NumberPhone varchar(50), penyNach float, iddocument int)
...
case when d.idtypedocument=13 then d.documentamount*0.03 else o.amountoperation end ,dbo.dateonly(d.documentdate),d.idtypedocument,d.iddocument
from document d  with (nolock) 
inner join period p  with (nolock) on d.idperiod=p.idperiod
and p.IdPeriod<=@idperiodKonech and  p.IdPeriod>=@idperiodNach and d.IdContract=@IdContract and d.idtypedocument not in (24,13,5,19,20,7,18,14,11,18,12)
...
inner join balance b on b.idbalance=o.idbalance and b.idaccounting=6
...
insert into #tmpAccount (NumberDoc,namedoc,IdPeriod, Period, Amount, DateDocument,typedocument, iddocument)
select case when d.idtypedocument=1 then isnull(d.DocumentNumber,'') else ''end, t.name, p.IdPeriod, case when len(ltrim(str(p.Month)))=1 then ltrim(str(p.Year))+'0'+ltrim(str(p.Month)) else ltrim(str(p.Year))+ltrim(str(p.Month)) end,
o.amountoperation*-1,dbo.dateonly(d.documentdate),d.idtypedocument,d.iddocument
from document d  with (nolock) 
inner join period p  with (nolock) on d.idperiod=p.idperiod
and p.IdPeriod<=@idperiodKonech and  p.IdPeriod>=@idperiodNach and d.IdContract=@IdContract and d.idtypedocument in (24)
inner join typedocument t  with (nolock) on t.idtypedocument=d.idtypedocument
inner join operation o on o.iddocument=d.iddocument
inner join balance b on b.idbalance=o.idbalance and b.idaccounting=6
 
insert into #tmpAccount (amount,namedoc,IdPeriod, Period,  DateDocument,indication,typedocument,iddocument)
select case when d.idtypedocument in (7,11,14) then o.amountoperation end, t.name, p.IdPeriod, case when len(ltrim(str(p.Month)))=1 then ltrim(str(p.Year))+'0'+ltrim(str(p.Month)) else ltrim(str(p.Year))+ltrim(str(p.Month)) end,
...
--inner join period p with (nolock) on d.idperiod=p.idperiod
--and p.IdPeriod<=@idperiodKonech and p.idperiod<>@DTPeriod and  p.IdPeriod>=@idperiodNach and d.IdContract=@IdContract and d.idtypedocument in (5)
--inner join typedocument t  with (nolock) on t.idtypedocument=d.idtypedocument
...
--inner join factuse f with (nolock) on f.idoperation=o.idoperation
--inner join period p with (nolock) on d.idperiod=p.idperiod
--and p.IdPeriod=@DTPeriod and d.IdContract=@IdContract and d.idtypedocument in (5)
--inner join typedocument t  with (nolock) on t.idtypedocument=d.idtypedocument
...
-----ДЛЯ НОВОГО ТАРИФА
--insert into #tmpAccount (namedoc,IdPeriod, Period, Amount, DateDocument,factuse,typefactuse,typedocument,iddocument)
--select case when isnull(f.idtypefu,0)=3 then left(t.name, 6)+'. без пок.' else t.name+' (НТ)' end, p.IdPeriod, case when len(ltrim(str(p.Month)))=1 then ltrim(str(p.Year))+'0'+ltrim(str(p.Month)) else ltrim(str(p.Year))+ltrim(str(p.Month)) end,
---sum(o.amountoperation),dbo.dateonly(d.documentdate), sum(f.factamount),f.idtypefu, d.idtypedocument,d.iddocument
--from document d  with (nolock) 
--inner join operation o with (nolock) on o.iddocument=d.iddocument
--inner join factuse f with (nolock) on f.idoperation=o.idoperation
...
endbalance=isnull(dbo.fGetLastBalance(idperiod,@idcontract,6),0), 
beginbalance=isnull(dbo.fGetLastBalance(idperiod-1,@idcontract,6),0),
...
update #tmpAccount
set tariff=388.89
where idperiod=@DTPeriod and namedoc like '%(НТ)%'
...
and go.IDContract = @IdContract
left join GMeter gm  with (nolock) on gm.IdGObject=go.IdGObject
and dbo.fGetStatusPU (@dEnd,gm.idgmeter)=1
left join StatusGmeter s with (nolock) on gm.IDStatusGmeter = s.IDStatusGmeter
left join typegmeter t with (nolock) on t.idtypegmeter=gm.idtypegmeter
left join StatusGobject st with (nolock) on go.IDStatusGobject = st.IDStatusGobject
...
from #tmpAccount t
inner join (select sum(amount) am, period from #tmpAccount where typedocument in (5,13) group by period) tt 
on tt.period=t.period

update #tmpAccount
set AmountNach=isnull(AmountNach,0)-isnull(tt.am,0)
from #tmpAccount t
inner join (select sum(amount)am, period from #tmpAccount where typedocument in (7,11,14,3,8) group by period) tt 
on tt.period=t.period

update #tmpAccount
set Amountpay=tt.am
from #tmpAccount t
inner join (select sum(amount)am, period from #tmpAccount where typedocument in (1) group by period) tt 
on tt.period=t.period

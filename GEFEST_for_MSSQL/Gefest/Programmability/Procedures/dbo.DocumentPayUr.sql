...
CREATE PROCEDURE [dbo].[DocumentPayUr] (@IDPeriod as int, 
	@IDBatch as int, 
	@IDContract as int, 
...
-- тех. обслуживание исключаем
-- потому как оплачиваем его отдельно
...
begin
	...
	values (@DocumentDate, round(@Amount1,6),  0,  @IDBalance,  @IDDocument,  1)
...
begin
	insert into Operation(DateOperation, AmountOperation , NumberOperation, IDBalance, IDDocument, IdTypeOperation)
	values (@DocumentDate, round(@Amount2,6),  2,  @IDBalance2,  @IDDocument,  1)
	if @@error<>0	set @blErr = 1
...
begin
	insert into Operation(DateOperation, AmountOperation , NumberOperation, IDBalance, IDDocument, IdTypeOperation)
	values (@DocumentDate, round(@Amount3,6),  3,  @IDBalance3,  @IDDocument,  1)
	if @@error<>0	set @blErr = 1
...
begin
	insert into Operation(DateOperation, AmountOperation , NumberOperation, IDBalance, IDDocument, IdTypeOperation)
	values (@DocumentDate, round(@Amount4,6),  4,  @IDBalance4,  @IDDocument,  1)
	if @@error<>0	set @blErr = 1

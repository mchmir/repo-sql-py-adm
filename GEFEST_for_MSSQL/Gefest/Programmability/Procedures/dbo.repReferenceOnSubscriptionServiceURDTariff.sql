...
create PROCEDURE [dbo].[repReferenceOnSubscriptionServiceURDTariff] (@idPeriod int) AS
...
update #AnalSal 
set M3NachPUDT=isnull(cubes,0),
AmountNachPUDT=isnull(ao,0)
from (select sum(f.FactAmount) cubes, sum(AmountOperation) ao
from FactUse f with (nolock) 
inner join indication i with (nolock) on i.idindication=f.idindication
	and isnull(f.IdTypeFU,0)=1 
...
inner join document d with (nolock) on d.iddocument=o.iddocument
	and idtypedocument=5
	and  f.IdPeriod=@idperiod
inner join pd with (nolock) on pd.iddocument=d.iddocument and pd.idtypepd=36 
inner join contract c with (nolock) on d.idcontract=c.idcontract and dbo.fGetIsJuridical(c.idperson,@idPeriod)=0 
where pd.value='1') qq
...
update #AnalSal 
set KGNachPUDT=isnull(cubes,0),
AmountKGNachPUDT=isnull(ao,0)
from (select sum(f.FactAmount) cubes, sum(AmountOperation) ao
from FactUse f with (nolock) 
inner join operation o   with (nolock) on o.idoperation=f.idoperation
inner join document d with (nolock) on d.iddocument=o.iddocument
...
update #AnalSal 
set M3NachPUUridDT=isnull(cubes,0),
AmountNachPUUridDT=isnull(ao,0)
from (select sum(f.FactAmount) cubes, sum(AmountOperation) ao
from FactUse f with (nolock) 
inner join indication i  with (nolock) on i.idindication=f.idindication
	and isnull(f.IdTypeFU,0)=1 
...
update #AnalSal 
set KGNachPUUridDT=isnull(cubes,0),
AmountKGNachPUUridDT=isnull(ao,0)
from (select sum(f.FactAmount) cubes, sum(AmountOperation) ao
from FactUse f with (nolock) 
inner join operation o   with (nolock) on o.idoperation=f.idoperation
inner join document d with (nolock) on d.iddocument=o.iddocument

...
CREATE PROCEDURE [dbo].[repSalesGasVDGOUL] (@idPeriod int, @Status int, @Path varchar(1000), @MonthName varchar(100) ) AS
---------------**************Отчет по реализации газа и услуг ВДГО по ЮЛ*********-------------------
SET NOCOUNT ON
...
Potreb float, NachZaGaz decimal(10,2), NachVDGO decimal(10,2), NachGP decimal(10,2), NachPeny decimal(10,2), AllNach decimal(10,2), Pay decimal(10,2),
SumPotreb float, SumNachZaGaz decimal(10,2), SumNachVDGO decimal(10,2), SumNachGP decimal(10,2), SumNachPeny decimal(10,2), SumPay decimal(10,2), SumAllNach decimal(10,2),SumCarry decimal(10,2), CarryPay decimal(10,2))
insert into #Tbl (IDContract, Account, NameObject,NamePerson,RNN,Address, Status, SaldoB, SaldoE, Potreb, NachZaGaz, NachVDGO, NachGP, NachPeny, Pay, AllNach,CarryPay)
...
isnull(Pay1.AmountPay,0)+isnull(Pay2.AmountPay,0)+isnull(Pay3.AmountPay,0)+isnull(Pay4.AmountPay,0)+isnull(Pay5.AmountPay,0)+isnull(Pay6.AmountPay,0)+isnull(Pay7.AmountPay,0) Pay,
...
left join(select c.idcontract, sum(o.amountoperation) AmountPay --UslAmountUrBEZNAL
from operation o  with (nolock)
inner join document d  with (nolock) on o.iddocument=d.iddocument
inner join batch bb   with (nolock) on d.idbatch=bb.idbatch
and d.idperiod=@idperiod and idtypedocument=1 
inner join balance b with (nolock) on b.idbalance=o.idbalance
inner join contract c with (nolock) on b.idcontract=c.idcontract 
and dbo.fGetIsJuridical(c.idperson,@idPeriod)=1 
	and b.idperiod=@idperiod
	and b.idaccounting=6 and o.idtypeoperation=1
group by c.idcontract) Pay7 on Pay7.idcontract=ct.idcontract
...
--set AllNach=(select NachZaGaz+NachVDGO+NachGP+NachPeny from #TBL)
--update #Tbl
...
if @status <3
	begin
		set @SQL=@SQL+' Account, isnull(NameObject,''-''),NamePerson,RNN, Address, replace(SaldoB,''.'','',''),  replace(Potreb,''.'','',''), replace(NachZaGaz,''.'','',''), '
		set @SQL=@SQL+'replace(NachVDGO,''.'','',''), replace(NachGP,''.'','',''), replace(NachPeny,''.'','',''), replace(AllNach,''.'','',''), '
		set @SQL=@SQL+'replace(Pay,''.'','',''), replace(CarryPay,''.'','',''), replace(SaldoE,''.'','',''), '
		set @SQL=@SQL+'replace(SumPotreb,''.'','',''), replace(SumNachZaGaz,''.'','',''), '
		set @SQL=@SQL+'replace(SumNachVDGO,''.'','',''), replace(SumNachGP,''.'','',''), replace(SumNachPeny,''.'','',''), replace(SumAllNach,''.'','',''), '
